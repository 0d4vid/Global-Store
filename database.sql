-- This file contains all the SQL commands to set up the 'global_store' database.
-- Please execute this script in your MySQL database management tool (e.g., phpMyAdmin).
...
DROP PROCEDURE IF EXISTS reapprovisionner_produit;
DROP PROCEDURE IF EXISTS chiffre_affaires_total;
DROP PROCEDURE IF EXISTS produits_plus_vendus;
DROP PROCEDURE IF EXISTS clients_fideles;
DROP PROCEDURE IF EXISTS lister_ventes_utilisateur;

DELIMITER //

CREATE PROCEDURE ajouter_categorie (IN p_libelle VARCHAR(100))
BEGIN
    INSERT INTO categorie(libelle) VALUES (p_libelle);
END //
...
CREATE PROCEDURE modifier_categorie (IN p_id_cat INT, IN p_libelle VARCHAR(100))
BEGIN
    UPDATE categorie SET libelle = p_libelle WHERE id_cat = p_id_cat;
END //

CREATE PROCEDURE supprimer_categorie (IN p_id_cat INT)
BEGIN
...
CREATE PROCEDURE modifier_stock_produit (IN p_id_prod INT, IN p_stock INT)
BEGIN
    UPDATE produit SET stock_actuel = p_stock WHERE id_prod = p_id_prod;
END //

CREATE PROCEDURE supprimer_produit (IN p_id_prod INT)
BEGIN
    DELETE FROM produit WHERE id_prod = p_id_prod;
END //

CREATE PROCEDURE ajouter_client (IN p_nom VARCHAR(100), IN p_tel VARCHAR(20))
BEGIN
    INSERT INTO client(nom_client, telephone) VALUES (p_nom, p_tel);
END //
...
CREATE PROCEDURE ajouter_utilisateur (IN p_nom VARCHAR(100), IN p_prenom VARCHAR(100), IN p_mail VARCHAR(100), IN p_mdp VARCHAR(255), IN p_role VARCHAR(20))
BEGIN
    -- It is critical to hash passwords before storing them.
    -- The original script stores plain text, which is a major security flaw.
    -- We will hash the password here. The PHP code must use password_hash().
...
BEGIN
    ...
    SELECT LAST_INSERT_ID() as id_vente;
END //

CREATE PROCEDURE ajouter_ligne_vente (IN p_id_vente INT, IN p_id_prod INT, IN p_qte INT, IN p_prix DECIMAL(10,2))
BEGIN
    INSERT INTO ligne_vente(id_vente, id_prod, quantite, prix_unitaire)
    VALUES (p_id_vente, p_id_prod, p_qte, p_prix);

    UPDATE produit SET stock_actuel = stock_actuel - p_qte WHERE id_prod = p_id_prod;
END //

CREATE PROCEDURE ajouter_paiement (IN p_id_vente INT, IN p_montant DECIMAL(10,2), IN p_mode VARCHAR(20))
BEGIN
    INSERT INTO paiement(id_vente, montant, mode_paiement) VALUES (p_id_vente, p_montant, p_mode);
    UPDATE vente SET statut = 'TERMINEE' WHERE id_vente = p_id_vente;
END //

CREATE PROCEDURE reapprovisionner_produit (IN p_id_prod INT, IN p_qte INT, IN p_id_fourn INT, IN p_id_user INT)
BEGIN
...
BEGIN
    SELECT SUM(montant) AS chiffre_affaires FROM paiement;
END //

CREATE PROCEDURE produits_plus_vendus ()
BEGIN
    SELECT p.nom, SUM(lv.quantite) AS total_vendu
    FROM ligne_vente lv
...
BEGIN
    SELECT c.nom_client, COUNT(v.id_vente) AS nombre_achats, SUM(c.point_fidelite) as total_points
    FROM client c
...
CREATE PROCEDURE lister_ventes_utilisateur(IN p_id_user INT)
BEGIN
    SELECT 
        v.id_vente,
        v.num_facture,
        ...
        v.statut,
        cl.nom_client,
        (SELECT SUM(lv.quantite * lv.prix_unitaire) FROM ligne_vente lv WHERE lv.id_vente = v.id_vente) AS total
    FROM vente v
...
-- RÉAPPROVISIONNEMENT
CALL reapprovisionner_produit(1, 10, 1, 3);
CALL reapprovisionner_produit(2, 3, 2, 3);

-- OUVERTURE SESSION DE CAISSE
INSERT INTO session_caisse(date_debut, fond_initial, id_user) VALUES (NOW(), 50000, 2);
