...
<div class="container mx-auto p-4 sm:p-6 lg:p-8">
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-500">Créer, modifier et gérer les comptes des employés.</p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg mb-8">
        <h2 class="text-2xl font-semibold text-gray-800 mb-6"><?php echo $edit_user ? "Modifier l'utilisateur" : "Ajouter un nouvel utilisateur"; ?></h2>
        <form action="utilisateurs.php" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="text" id="nom_user" name="nom_user" value="<?php echo htmlspecialchars($edit_user['nom_user'] ?? ''); ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                </div>
                <div>
                    <label for="prenom_user" class="block mb-2 text-sm font-medium text-gray-900">Prénom</label>
                    <input type="text" id="prenom_user" name="prenom_user" value="<?php echo htmlspecialchars($edit_user['prenom_user'] ?? ''); ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                </div>
                <div>
                    <label for="mail_user" class="block mb-2 text-sm font-medium text-gray-900">Email</label>
                    <input type="email" id="mail_user" name="mail_user" value="<?php echo htmlspecialchars($edit_user['mail_user'] ?? ''); ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                </div>
                <div>
                    <label for="mot_de_passe" class="block mb-2 text-sm font-medium text-gray-900">Mot de passe</label>
                    <input type="password" id="mot_de_passe" name="mot_de_passe" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" <?php echo !$edit_user ? 'required' : ''; ?>>
                    <?php if ($edit_user): ?>
                ...
                <div class="lg:col-span-2">
                    ...
                    <select id="role" name="role" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        <option value="admin" <?php echo (isset($edit_user) && $edit_user['role'] == 'admin') ? 'selected' : ''; ?>>Administrateur</option>
                        <option value="vendeur" <?php echo (isset($edit_user) && $edit_user['role'] == 'vendeur') ? 'selected' : ''; ?>>Vendeur</option>
                        <option value="stock" <?php echo (isset($edit_user) && $edit_user['role'] == 'stock') ? 'selected' : ''; ?>>Gestionnaire de Stock</option>
                    </select>
                </div>
            </div>
            <div class="flex items-center mt-6">
                <button type="submit" class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                    <?php echo $edit_user ? 'Mettre à jour l\'utilisateur' : 'Ajouter l\'utilisateur'; ?>
                </button>
                <?php if ($edit_user): ?>
                    <a href="utilisateurs.php" class="ml-4 py-2.5 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200">Annuler</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg">
        <h2 class="text-2xl font-semibold text-gray-800 mb-6">Liste des utilisateurs</h2>
                    ...
                    <?php else: ?>
                        <?php foreach ($users as $user): ?>
                            <tr class="bg-white border-b hover:bg-gray-50">
                                <td class="px-6 py-4 font-medium text-gray-900"><?php echo htmlspecialchars($user['id_user']); ?></td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="utilisateurs.php?edit_id=<?php echo $user['id_user']; ?>#form" class="font-medium text-blue-600 hover:underline">Modifier</a>
                                    <?php if ($user['id_user'] != $_SESSION['user_id']): ?>
                                        <a href="utilisateurs.php?delete_id=<?php echo $user['id_user']; ?>" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet utilisateur ? Cette action est irréversible.');" class="font-medium text-red-600 hover:underline ms-3">Supprimer</a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
